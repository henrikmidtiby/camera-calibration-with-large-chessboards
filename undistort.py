...
def main():
    parser = argparse.ArgumentParser(
        description="undistort images, if no arguments are given, /input and /output in current folder are used"
    )
    parser.add_argument(
        "-i",
        "--input",
...
def undistort_images(list_input, output, calibration_file):
    ...
    if len(d) == 4:  # fisheye model
        distortion = np.array([[d[0]], [d[1]], [d[2]], [d[3]]]).astype(float)

        for fname in list_input:
            img = cv2.imread(str(fname))
            h, w = img.shape[:2]
            map1, map2 = cv2.fisheye.initUndistortRectifyMap(
                matrix, distortion, np.eye(3), matrix, (w, h), cv2.CV_16SC2
            )
        ...
        distortion = np.array([[d[0]], [d[1]], [d[2]], [d[3]], [d[4]]]).astype(float)
        ...
        sys.exit(
            "Something is wrong with the calibration file... Redo calibration and try again"
        )
